...
class UiSetupFeatures:
    ...
    def _setup_sidebar(self):
        ...
        github_icon_size = 22
        github_button = QPushButton("   Give a Star!")
...
<circle cx='12' cy='12' r='10'/><path d='M12 16v-4'/><path d='M12 8h.01'/></svg>"""
        ...
        def _on_quality_info_badge_clicked():
            if "Token Required" in self.quality_info_badge.text():
                self.open_settings_page()
                QTimer.singleShot(50, lambda: getattr(self.settings_page, "highlight_media_user_token", lambda: None)())
        self.quality_info_badge.clicked.connect(_on_quality_info_badge_clicked)
